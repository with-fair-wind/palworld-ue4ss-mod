...
public:
    ...
    [[nodiscard]] auto request_is_current(const std::uint64_t requestGeneration) const noexcept
        -> bool {
...
/**
 * @brief A value-only request bound to the world generation in which it was submitted.
 */
...
/**
 * @brief Checks whether a non-edit request still belongs to the accessible current world.
 */
[[nodiscard]] inline auto request_can_run(const WorldBoundRequest& request,
                                          const WorldSessionState& session) noexcept -> bool {
    return session.request_targets_current_world(request.worldGeneration);
}
}  // namespace skill_editor
