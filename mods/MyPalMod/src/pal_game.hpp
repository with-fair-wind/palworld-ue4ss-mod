...
inline auto add_passive(UObject* pal, const std::string& skillId) -> void
{
    if (pal == nullptr)
    {
        return;
    ...
    const std::wstring wide(skillId.begin(), skillId.end());
    struct
    {
        FName AddSkill;
        FName OverrideSkill;
...
inline auto remove_passive(UObject* pal, const std::string& skillId) -> void
{
    if (pal == nullptr)
    {
        return;
...
inline auto read_pal_passives(UObject* pal) -> void
{
    if (pal == nullptr)
    {
        return;
