...
60 秒低频校准只在没有活动材料会话时运行，作为事件遗漏时的安全兜底。已登记但尚未加载的普通箱子不会再使
整次目录校准失败：已加载部分立即可用，未加载部分从本次联合排除，并按 1、2、4 秒逐步退避、最高 30 秒重试。
同一世界内重新开启只安排既有的一次管理器目录校准，不增加线程、全局扫描、槽位扫描或逐帧任务；失败仍按
结果分类调度，关闭状态保持零资源 Hook 和零目录发现。

原版建筑菜单的 `OnOpenMenu` 和制作模型的 `Initialize` 都在原函数执行前建立短生命周期资源联合，使首次
图标/配方资格计算即可读取共享材料。制作和建造都只向本地主背包 `InventoryMultiHelper` 注入去重后的普通
箱子，不再同时扩展 Helper 与据点模块。制作与建造不能并存，新的前台操作会
先恢复旧联合再抢占。写入后会调用原版 `OnRep`、重读并验证每个注入容器恰好出现一次；数量或顺序异常会回滚并
在本世界安全停用对应能力。`StartProduction` 只确认会话仍在活动；制作界面由
`PalUserWidget:OnClosed` 精确识别 `PalHUDDispatchParameter_ConvertItem` 后释放，退出建造模式时释放建造会话。
建筑菜单 `Setup` 完成后只向该建造模型发送一次原生 `OnUpdateInventory(Container)` 事件，使首次资格缓存立即
重算；不会重复触发 Helper 的 `OnRep_Containers`，也不会增加逐物品或逐帧 Hook。
跨帧只保存 GUID、对象全名和序列账本，不保存 `UObject*` 或 Unreal 数组地址。高频预览 Hook 只更新
固定大小会话状态，不执行反射、目录扫描、数组改写或日志输出。若首次资格回调早于目录初始化，会在该次
pre-hook 中同步执行一次有界目录 bootstrap 并建立联合，不需要先打开另一座建筑，也不会进入逐帧重试。

只有终端、尚无普通仓储模块的据点仍计入据点数量，但不会贡献材料；建造所在据点优先由进入/离开据点事件确认，
必要时只做一次最近据点回退。界面仅发布纯值诊断：前台操作、单一联合入口、当前据点确认状态以及最近目录/联合
耗时、最近成功目录耗时、本世界峰值、尝试次数和暂未加载容器数，不提供逐帧材料数量预览。
...
- 1.6.8 将爪钩与资源共享改为进程内动态开关，并把制作/建造收敛为互斥使用同一个玩家材料 Helper；
  ...
  世界安全停用。
- 1.6.9 将目录空闲兜底降为 60 秒，未加载箱子改为部分成功与指数退避，并在建筑菜单 `Setup` 后仅发送一次
  原生库存更新事件，修复首次打开建造菜单材料足够但图标仍灰显的问题。
