# PalworldEditor 1.4.1 — Palworld 物品与帕鲁技能编辑器
...
| **目标帕鲁** | 每帧识别 Q/E 当前选中的、下一次会被投出战斗的队伍帕鲁 |
...
| **安全修改** | 游戏线程 FIFO 执行，目标切换时拒绝旧请求；操作后重读，替换失败时恢复原技能 |
...
1. 确保队伍中有帕鲁，并用 Q/E 选择下一只待出战帕鲁。无需打开帕鲁盒子或详情页；
   窗口顶部显示 `当前待出战帕鲁：<CharacterID>`。
2. 被动技能区域：
   - 当前被动逐行显示；
   - 点击“替换”后从可搜索下拉框中选择并确认；
   - 点击“删除”移除已有被动；
   - 未满 4 个时可“新增被动技能”。
3. 主动技能区域固定显示 3 个 `EquipWaza` 槽位：
   - 已装备槽位可替换或清空；
   - 第一个空槽可选择并装备新技能。
4. 点击“刷新技能列表”可重新加载技能目录并重读当前技能。

下拉框只在点击确认时提交修改；选择候选本身不会立刻写入游戏。已经拥有/装备的技能会从候选中隐藏。
每次修改都在游戏线程执行并重读实际状态；替换未生效时会尝试恢复完整原状态。
如果请求入队后玩家用 Q/E 切换了目标，该请求会被拒绝，不会写到新目标。技能目录本地化始终使用
`PalPlayerInventoryData` 作为稳定世界上下文，不再使用帕鲁参数对象。
