# PalworldEditor 1.5.2 — Palworld 物品、帕鲁技能与据点资源编辑器
...
4. 主动技能区域固定显示 3 个 `EquipWaza` 槽位：
   - 已装备槽位可替换或清空；
   - 第一个空槽可选择并装备新技能。
5. 进入存档后 mod 会每两秒自动重试一次完整技能目录，成功后停止重试；点击
   “刷新技能列表”可随时立即重新加载主动技能、被动技能及当前游戏语言名称。
...
启动时的一次物品扫描和技能目录加载/失败重试保持不变；它们是初始化工作，不属于已经移除的常驻逐帧解析。
...
1.5.2 的当前帕鲁常驻性能修复没有改变上述据点发现、预览、临时联合或实际扣料行为。
