...
| **爪钩枪无冷却** | 开关：开启后爪钩枪射击无冷却（改 `PalWeaponBase.CoolDownTime`，对所有武器生效），持久化到 config.ini；重新装备后需重新点开关 |
...
- 1.6.1 修复同一世界内关闭后重新开启资源共享的生命周期；1.6.2 移除确认帕鲁后的 250 毫秒
  ...
  会话期间的目录校准；1.6.4 增加被动技能四词条预设的单请求差量应用与失败回滚，不增加常驻工作；1.6.7
  增加爪钩枪无冷却开关（写 `PalWeaponBase.CoolDownTime`，与参考 mod 一致），按需游戏线程应用、世界就绪重应用。
