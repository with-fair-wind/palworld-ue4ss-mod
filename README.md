# PalworldEditor 1.6.8 — Palworld 物品、帕鲁技能与据点资源编辑器
...
图标/配方资格计算即可读取共享材料。制作只向本地主背包 `InventoryMultiHelper` 注入去重后的普通箱子；
建造只向当前据点的仓储模块注入，不再同时扩展 Helper 与所有据点模块。制作与建造不能并存，新的前台操作会
先恢复旧联合再抢占。写入后会调用原版 `OnRep`、重读并验证每个注入容器恰好出现一次；数量或顺序异常会回滚并
在本世界安全停用对应能力。`StartProduction` 只续租，制作会话由 1.5 秒空闲租约释放；退出建造模式时释放
建造会话。跨帧只保存 GUID、对象全名和序列账本，不保存 `UObject*` 或 Unreal 数组地址。高频预览 Hook 只更新
...
- 1.6.8 将爪钩与资源共享改为进程内动态开关，并把资源共享收敛为“制作 Helper / 当前据点模块”两个互斥的
  单一消费面；首次资格计算前完成有界 bootstrap，联合写后验证恰好一次，异常时回滚并按世界安全停用。
