...
爪钩与资源共享开关都只在本次游戏进程有效，每次 DLL 加载默认关闭，不读取或写入 `config.ini`。爪钩功能
等待 Common 主背包就绪后只执行一次精确目标扫描；未找到目标时仅允许用户显式重试，关闭和 LoadMap 前在
游戏线程按账本恢复。1.6.8 同时引入资源单一消费面、首次资格同步 bootstrap、联合序列验证与纯值耗时诊断。
热卸载前必须先关闭两个功能，析构不访问 Unreal；关闭状态和空闲状态不增加常驻工作。
...
游戏内应看到 `PalworldEditor loaded (v1.6.8)`。除物品、技能和世界切换回归外，还要验证：
