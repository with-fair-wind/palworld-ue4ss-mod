...
这是一个面向 **Palworld 1.0** 的 UE4SS C++23 mod。当前 mod 名为 `PalworldEditor`（版本 1.4.0），通过
UE4SS GUI 提供：

- 运行时物品目录和当前语言名称搜索；
- 给予物品、读取主背包和修改槽位数量；
- 自动识别当前查看的帕鲁，或使用扫描结果手动选择；
- 被动技能新增、替换、删除；
...
- `pal_game.hpp`：背包、物品定义、帕鲁对象和诊断扫描的反射访问；
...
- `dllmain.cpp`：`PalworldEditorMod` 生命周期、ImGui、当前查看目标钩子和线程间请求交接。

ImGui 回调只读写标准库 UI 状态、原子请求标志以及互斥锁保护的快照/参数。所有 UObject 反射读取和修改都在
`on_update()` 所在游戏线程执行；`on_unreal_init()` 只进行初始化查询和钩子注册。UObject 指针是非拥有句柄，
不得跨生命周期长期持有。
...
游戏内验证时，UE4SS 控制台应出现 `PalworldEditor loaded (v1.4.0)`；打开 `PalworldEditor` 页签后验证物品、
背包、目标帕鲁识别，以及主动/被动技能编辑。反射签名和 UFunction 参数布局来自 Palworld 1.0，游戏更新后可能
